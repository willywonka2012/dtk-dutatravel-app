...
import { IonContent,IonIcon,IonText, IonModal,IonAlert,IonLoading,IonToolbar,IonPopover, IonItem, IonSearchbar ,IonPage,IonHeader,IonCard,IonLabel,IonDatetime,IonCol,IonCardContent, IonButton, IonRow, IonGrid, IonList, isPlatform} from '@ionic/react';
...
const Search: React.FC<SearchProps> = ({history}) => {
  ...
  const [selectedStartDateString, setSelectedStartDateString] = useState<string>(localStorage.filterStartDate?localStorage.filterStartDate:today.toISOString());
  const [selectedEndDateString, setSelectedEndDateString] = useState<string>(localStorage.filterEndDate?localStorage.filterEndDate:new Date(new Date().setDate(new Date().getDate() + 6)).toISOString());
  const [searchText, setSearchText] = useState<string>(localStorage.filterSearchText?localStorage.filterSearchText:'');

  const [selectedStartDate, setSelectedStartDate] = useState<Date>(new Date(selectedStartDateString));
  const [selectedEndDate, setSelectedEndDate] = useState<Date>(new Date(selectedEndDateString));

  const [searchDateString, setSearchDateString] =
    useState<string>(`
      ${selectedStartDate.getDate()} ${monthNames[selectedStartDate.getMonth()]} ${selectedStartDate.getFullYear()}
       -
      ${selectedEndDate.getDate()} ${monthNames[selectedEndDate.getMonth()]} ${selectedEndDate.getFullYear()}
    `);
  // alert
  ...
  const getTourSearch = (offset=0,limit=20) => {
    ...
    const dateTo  = EndDate.getFullYear()+'-'+(EndDate.getMonth()+1)+'-'+EndDate.getDate();
  ...
  return (
    <IonPage>
      <IonHeader translucent >
          <DefaultToolbar
          title={searchText!==null || searchText == ''?searchText:'Cari Paket Tour'} color="primary" backButtonRoute="/tour"
          />
          {/* <ClickableSearchBox
          ...
          <IonToolbar color="primary" className="searchToolbar">
            <IonGrid onClick={() => setShowPopover(true)}>
              <IonRow>
              <IonCol size="11">
                <IonLabel><small>{searchDateString}</small></IonLabel>
                </IonCol>
                <IonCol size="1">
      ...
      <IonContent fullscreen={true} class="tourSearch">
        ...
        {TourProductSearchList !== null?TourProductSearchList.map((dataItem:any,index:any) => (
          <IonRow className="ion-margin-bottom" key={dataItem.code} onClick={()=>history.push(`/tourDetail/${dataItem.title.replace(/\s+/g, '-')}/${dataItem.code}`)}>
            <IonCol size="12" className="ion-padding">
              <img src={ImageBasePath+dataItem.image} width="100%" alt=""/>
              <IonText color="medium"><p className="light-gray ion-no-margin"><small>Tour</small></p></IonText>
              <IonText color="dark"><p className="ion-no-margin">{dataItem.title}</p></IonText>
              <IonText color="medium"><p className="ion-no-margin ion-margin-bottom">{rupiah(dataItem.price)}</p></IonText>
        ...
        >
          <IonItem>
          <IonSearchbar placeholder={searchText!==null?searchText:'Mau kemana hari ini'} mode="ios" className="ion-no-padding" onClick={() => setShowModal(true)}></IonSearchbar>
          </IonItem>
          <IonItem>
            <IonLabel>Tanggal Berangkat</IonLabel>
            <IonDatetime min={new Date().toISOString()} max={new Date(new Date().setFullYear(new Date().getFullYear() + 10)).toISOString()} displayFormat="DD MMMM YYYY" value={selectedStartDateString} onIonChange={e => changeStartDate(e.detail.value!)} doneText="Pilih" cancelText="Batal"></IonDatetime>
          </IonItem>
          <IonItem>
            <IonLabel>Tanggal Kembali</IonLabel>
            <IonDatetime min={new Date().toISOString()} max={new Date(new Date().setFullYear(new Date().getFullYear() + 10)).toISOString()} displayFormat="DD MMMM YYYY" value={selectedEndDateString} onIonChange={e => changeEndDate(e.detail.value!)} doneText="Pilih" cancelText="Batal"></IonDatetime>
          </IonItem>
          <IonItem>
              <IonButton size="large" onClick={() => getTourSearch()}>Cari Paket Tour</IonButton>
          </IonItem>
        </IonPopover>
      ...
      <IonModal isOpen={showModal} cssClass='my-custom-class'>
        <IonContent>
          <DefaultToolbar
            title="Cari Lokasi Tujuan" color="primary" backButtonRoute={closeModal}
            />
            <IonSearchbar
            value={searchText}
            placeholder={searchText!==null || searchText == "null" ?searchText:'Mau kemana hari ini'}
