...
import { IonContent,IonToolbar,IonCard,IonCardContent,IonFooter,IonButton,IonPage,IonHeader, IonRow, IonGrid, IonButtons, IonBackButton, IonTitle, IonCol, IonText, IonPopover, IonAlert, IonLoading, isPlatform} from '@ionic/react';
...
interface TourOrderProps extends OwnProps, StateProps, DispatchProps,RouteComponentProps { };
const TourOrder: React.FC<TourOrderProps> = ({
    ...
    if(localStorage.TourOrderPaxData !== JSON.stringify(DataArray)){
      ...
      for (let index = 0; index < DataArray.length; index++) {
        ...
        PaxFullDataArray.push({
          PaxMaturity:PaxMaturity.includes("Bayi")?"Infant":PaxMaturity.includes("Anak")?"Child":"Adult",
          PaxTitel:'',
          PaxFirstName:'',
          PaxLastName:'',
          PaxNation:'',
      ...
      localStorage.setItem("TourOrderPaxFullData",JSON.stringify(PaxFullDataArray));
    }
  }, [TourBookingNumberOfAdult,TourBookingNumberOfChild,TourBookingNumberOfInfant])
  React.useEffect(()=>{
  ...
  const OrderValidation = () => {
    ...
    const TOPFD = localStorage.TourOrderPaxFullData?JSON.parse(localStorage.TourOrderPaxFullData):undefined;
    if(TOPFD){
  ...
  const OrderSubmit = () => {
    ...
    MyData.append("tourBookingDetailModels.TourBookingContactTitle", JSON.parse(localStorage.TourOrderOrderPerson).OrderPersonTitel);
    MyData.append("tourBookingDetailModels.TourBookingContactName", JSON.parse(localStorage.TourOrderOrderPerson).OrderPersonFullName);
    MyData.append("tourBookingDetailModels.nation_pemesan", JSON.parse(localStorage.TourOrderOrderPerson).OrderPersonNationCode);
    MyData.append("tourBookingDetailModels.nohp_pemesan", JSON.parse(localStorage.TourOrderOrderPerson).OrderPersonPhoneNumber);
    MyData.append("tourBookingDetailModels.TourBookingContactEmail", JSON.parse(localStorage.TourOrderOrderPerson).OrderPersonEmail);
    MyData.append("tourBookingDetailModels.TourBookingSpecialRequest", localStorage.TourOrderSpecialRequest||null);
    if(TOPFD){
      for (let index = 0; index < TOPFD.length; index++) {
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestMaturity`, TOPFD[index].PaxMaturity);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestFirstName`, TOPFD[index].PaxFirstName);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestLastName`, TOPFD[index].PaxLastName);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestTitle`, TOPFD[index].PaxTitel);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestNationality`, TOPFD[index].PaxNation);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestBirthOfDate`, TOPFD[index].PaxBirthday);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestPassportNumber`, TOPFD[index].PaxPassportNumber);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestPassportIssuedDate`, TOPFD[index].PaxPassportPublishDate);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestIssuedCountry`, TOPFD[index].PaxPassportPublishCountry);
        MyData.append(`TourBookingGuestList[${index}].TourBookingGuestExpiryDate`, TOPFD[index].PaxPassportExpiredDate);
      }
    }
    if(TOAON){
      for (let index = 0; index < TOAON.length; index++) {
        MyData.append(`tourBookingAddOnList[${index}].AddOnId`, TOAON[index].id);
        MyData.append(`tourBookingAddOnList[${index}].NumberOfAddOn`, TOAON[index].number);
      }
    }
  ...
  return (
    <IonPage>
      ...
      <IonHeader>
        <IonToolbar color="primary">
          <IonButtons slot="start">
            <IonBackButton defaultHref="/tourBookingSeriesSelectPax"></IonBackButton>
          </IonButtons>
          <IonTitle>
            Isi Data
          </IonTitle>
      ...
      <IonContent fullscreen={true} class="TourOrder">
        ...
        <TourOrderOrderPerson name={UserData.name} email={UserData.email}></TourOrderOrderPerson>
        <TourOrderBuyerData
          domesticTourType={TourProductDetail.TourProductModels.ProductTourTourType==="Internasional"?false:true}
        ></TourOrderBuyerData>
        <TourOrderAddOn
      ...
      >
        ...
        <IonText><h6><b>Apakah pesanan Anda benar?</b></h6></IonText>
        <IonText color="medium">
          <p>Detail pesanan tidak dapat diubah setelah melanjutkan ke halaman pembayaran. Tetap lanjutkan?</p>
        </IonText>
        <IonButton color="primary"  size="large" className="text-transform-none" expand="block" onClick={()=>OrderSubmit()}>
        Ya, lanjutkan
        </IonButton>
        <IonButton color="medium" size="large" className="text-transform-none" expand="block" onClick={() => setSubmitWarning(false)}>
        Periksa Kembali
        </IonButton>
        </div>
      ...
      <IonFooter>
        <IonCard className="ion-no-margin ion-no-padding footerPrice">
          <IonGrid>
            <IonRow>
              <IonCol size="7">
                <IonText color="medium"><small>Harga yang harus dibayar ({TourBookingNumberOfPaxTotal} pax)</small></IonText>
                <IonText><h4 className="ion-no-margin">{rupiah(GrandTotal||0)}</h4></IonText>
              </IonCol>
              <IonCol size="5">
                <IonButton expand="block" className="text-transform-none" onClick={() => setSubmitWarning(true)}>Beli Sekarang</IonButton>
              </IonCol>
            </IonRow>
          </IonGrid>
        </IonCard>
